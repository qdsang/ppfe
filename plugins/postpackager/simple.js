...
var defaultSetting = {
    ...
    headTag: "</head>",
    bodyTag: "</body>",
    output: '${filepath}/pkg/auto_combine_${hash}'
};
 ...
 * @returns {Array}
 */
function autoCombine(resList, ret, conf, settings, opt, releasePath) {
    var list = [];
    var toCombine = [];
    var fileExt;
    ...
    function flushCombine() {
        ...
        if (toCombine.length !== 0) {
            ...
            if (combineCache[hash]){
                ...
                var subpath = settings.output.replace('${index}', combineCount).replace('${filepath}', releasePath)
...
module.exports = function (ret, conf, settings, opt) { //打包后处理
    ...
    fis.util.map(ret.src, function (subpath, file) {
        if (file.isHtmlLike && file.noMapJs !== false) { //类html文件
            ...
            var releasePath = path.dirname(file.release);
            ...
            if (settings.autoCombine) {
                jsList = autoCombine(jsList, ret, conf, settings, opt, releasePath);
                cssList = autoCombine(cssList, ret, conf, settings, opt, releasePath);
            }
