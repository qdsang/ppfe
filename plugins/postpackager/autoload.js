...
module.exports = function (ret, conf, settings, opt) { //打包后处理
     ...
     * @returns {XML|string|void}
     */
    function injectSiteAsync(content, subfile) {
        var filepath = path.dirname(subfile.release);
        function genSiteAsyncMap() {
            var asyncList = [];
            fis.util.map(ret.map.res, function (id) {
                asyncList.push(ret.ids[id]);
            });
            var subpath = (filepath + '/pkg/map.js').replace(/^\//, '');
     ...
     * @returns {*}
     */
    function injectAsync(asyncList, content, usedSync, subfile) {
        var filepath = path.dirname(subfile.release);
    ...
    function injectAutoLoad(file, include) {
        ...
        if (settings.useSiteMap) {
            content = injectSiteAsync(content, file);
        } else {
            content = injectAsync(asyncList, content, usedSync, file);
        }
